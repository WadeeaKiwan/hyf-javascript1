...
function printAdvertisement(array) {
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    if (counter === array.length - 2) {
      advertisement = advertisement + ` ` + array[index + 1] + `s and ` + array[index + 2] + `s.`;
...
//  or
function printAdvertisement2(array) {
  let f = ``;
  for (let index = 0; index < array.length - 2; index++) {
    f += `${array[index]}s, `;
  }
  const advertisement = `Amazing Joe's Garage, we service ${f}${array[array.length - 2]}s and ${
    array[array.length - 1]
  }s.`;
  console.log(advertisement);
}

printAdvertisement2(vehicles);
